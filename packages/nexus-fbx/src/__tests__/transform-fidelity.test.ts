...
describe("FBX transform fidelity", () => {
  ...
  it("marks mirrored transforms and shared instances on import", () => {
    ...
    const instancedBuffer = new FBXBinaryWriter().writeNodes([
      ...
      new FbxExportNode(
        ...
        [
          new FbxExportNode(
            ...
            ["Vertices: 0,0,0,1,0,0,0,1,0", "PolygonVertexIndex: 0,1,-3", "Normals: 0,1,0,0,1,0,0,1,0"],
          ),
          new FbxExportNode("Model", [2, "Model::InstanceA", "Model"]),
          new FbxExportNode("Model", [3, "Model::InstanceB", "Model"]),
        ],
      ),
      new FbxExportNode("Connections", [], ['C: "OO", 1, 2', 'C: "OO", 1, 3']),
      new FbxExportNode("Takes"),
    ]);
    const instanced = importer.read(instancedBuffer, "instanced.fbx").scene;
  ...
  it("exports transform stack properties and keeps one shared geometry for instanced nodes", () => {
    const buffer = new FBXExporter().write(createInstancedScene());
    const scene = new FBXImporter().read(buffer, "instanced-binary.fbx").scene;
    const instanceNodes = scene.rootNode.children[0]?.children ?? [];
    const firstTransformStack = JSON.parse(String(instanceNodes[0]?.metadata?.["fbx:transformStack"]?.data ?? "{}"));

    expect(new TextDecoder("ascii").decode(buffer.slice(0, 23))).toBe("Kaydara FBX Binary  \0\x1a\0");
    expect(scene.meshes).toHaveLength(1);
    expect(instanceNodes.map((node) => node.name)).toEqual(["InstanceA", "InstanceB"]);
    expect(firstTransformStack.rotationPivot).toEqual({ x: 0.5, y: 0, z: 0 });
    expect(firstTransformStack.scaling).toEqual({ x: -1, y: 1, z: 1 });
  });
});
